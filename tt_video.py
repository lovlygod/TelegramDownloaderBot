...
async def tt_videos_or_images(url):
    ...
    if len(video_id_from_url) > 0:
        ...
        if r1.status_code in [301, 302, 307, 308]:
            ...
            if r1.url != url:
                # Пытаемся извлечь ID из нового URL
                new_video_id = findall('https://www.tiktok.com/@.*?/video/(\d+)', str(r1.url))
                if new_video_id:
                    video_id = new_video_id[0]
                else:
                    # Ищем в теле ответа
                    video_id = findall(
                        'a href="https://\w{1,3}\.tiktok\.com/(?:@.*?/video|v)/(\d+)', r1.text)
                    if video_id:
                        video_id = video_id[0]
                    else:
                        # Пробуем другие возможные паттерны
                        video_id = findall("video&#47;(\d+)", r1.text)
                        if video_id:
                            video_id = video_id[0]
                        else:
                            return BaseException('Could not extract video ID from redirect')
            else:
                # Если URL не изменился, но статус 30x, пробуем разные паттерны
                video_id = findall(
                    'a href="https://\w{1,3}\.tiktok\.com/(?:@.*?/video|v)/(\d+)', r1.text)
                if video_id:
                    video_id = video_id[0]
                    ...
                    if video_id:
                        ...
                        return BaseException('Could not extract video ID from page content')
        elif r1.status_code == 403:
            video_id = findall("video&#47;(\d+)", r1.text)[0]
        elif r1.status_code == 200:
            # Пробуем извлечь ID из содержимого страницы
            video_id = findall('video\/(\d+)', r1.text)
            if video_id:
                video_id = video_id[0]
            else:
